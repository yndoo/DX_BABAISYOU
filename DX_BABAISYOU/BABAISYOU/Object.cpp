...
void AObject::Tick(float _DeltaTime)
{
...
bool AObject::CanGoNextAll(Index2D _Next, EInputDir _Dir)
{
	...
	for (Iter = GMapManager->Graph[_Next.X][_Next.Y].begin(); Iter != GMapManager->Graph[_Next.X][_Next.Y].end(); Iter++)
	{
		...
		if ((*Iter)->Info->MyObjectiveType[EObjectType::STOP] == true)
		{
			return false;
		}
		if (
			(*Iter)->Info->MyObjectiveType[EObjectType::PUSH] == true ||
...
void AObject::PushTrueHistory()
{
	...
	ObjectiveStack.push(Info->MyObjectiveType);
	int a = 0;
}
...
void AObject::PushFalseHistory()
{
	...
	//ObjectiveStack.push(Info->MyObjectiveType);	// 얘는 필요없나?
}
